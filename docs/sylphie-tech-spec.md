...
Hot residual layer: 30s TTL, 0.80/cycle decay. Output formatted between sentinel markers ("=== YOUR COMPLETE KNOWLEDGE ON THIS TOPIC ===") and injected as system prompt for deliberation.

### 4.5 Episodic Memory

Ring buffer capacity 50. Encoding gate fires only when **both** attention ≤ 0.15 *and* arousal ≤ 0.15 — i.e., calm, focused states get encoded; frantic ones don't. Persisted to TimescaleDB checkpoint. Queried by Jaccard > 0.70.
...
A separate Neo4j instance indexed by ts-morph parses Sylphie's own source. Exposes 7 MCP tools to Claude Code (`mcp__sylphie-pkg__*`):
