...
## Memory: four knowledge graphs, ACT-R confidence, and a working memory that selects
...
The forward direction follows the thesis. Today, Type 2 deliberation runs through multiple LLMs in a focus-group pattern. The path to closing the loop is replacing those LLMs with three specialized tensor pipelines (pragmatist, conservative, advocate) that have already shadowed the LLM panel long enough to earn promotion. When that lands, the last LLM dependency in the cognitive path is gone, and the share-of-decisions-without-an-LLM number — once it's instrumented to report live (Phase 2) — has somewhere to actually land.

## Why it matters

Most agent products are LLM wrappers that get smarter by switching to a bigger LLM. Sylphie is built so the LLM is the scaffolding, not the load-bearing structure — drives, predictions, evaluations, and graduated procedures are the load-bearing structure. The metric the architecture is built to surface is **the share of decisions made without calling a language model**, and it's supposed to climb. Live instrumentation of that number is on the Phase 2 list, not on a dashboard yet — but the architecture is what makes "slowly, then faster, then mostly not at all" a coherent trajectory rather than a slogan.
...
| Drive rules table + RLS policy (startup verification) | `apps/drive-server/src/main.ts` |
| Drive engine package | `packages/drive-engine/` |
| Decision-making package (Cortex) | `packages/decision-making/` |
| Dual-process arbitration (Type 1 / Type 2 / SHRUG) | `packages/decision-making/src/arbitration/` |
| Predict step (expected drive deltas) | `packages/decision-making/src/prediction/` |
| Evaluate step (MAE computation) | `packages/decision-making/src/confidence/confidence-updater.service.ts` |
| Working memory selector (5-signal activation) | `packages/decision-making/src/working-memory/` |
| Episodic memory ring buffer | `packages/decision-making/src/episodic-memory/` |
| Pathology detectors (5 attractors) | `packages/decision-making/src/monitoring/attractor-monitor.service.ts` |
| Tensor cognition sidecar (Python FastAPI) | `packages/cognition-service/` (port 8431) |
| Learning package (consolidation, reflection) | `packages/learning/` |
| Planning package (opportunity → procedure) | `packages/planning/` |
| Communication subsystem | `apps/sylphie/src/services/communication.service.ts` |
| Supervisor (DeepSeek meta-evaluator) | `packages/supervisor/` |
...
| Event bus types + boundary map | `packages/shared/src/types/event.types.ts` |
| TimescaleDB events hypertable | postgres `timescaledb` port 5433, table `events` |
| Postgres (auth, drive_rules) | postgres port 5434 |
| Perception sidecar (YOLO + MediaPipe + Moondream2) | `packages/perception-service/` (port 8430) |
| Frontend dashboard | `frontend/` (Vite + React 18 + Zustand + MUI) |
| Inner Monologue panel (verbatim event reader) | `frontend/src/components/InnerMonologue/` |
