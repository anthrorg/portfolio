...
`memory-pkg` is original to drift-detector — no earlier version exists in the sylphie repo. The complementary `codebase-pkg` package in the same monorepo *was* ported from sylphie's [`sylphie-pkg`](/cutting-edge-tech/sylphie-pkg), and is what the (currently dormant) KG retrieval tier consults at `bolt://localhost:7688`. Both packages now live in standalone repos under the `Sylphie-Labs` org — [`@sylphie-labs/memory-pkg`](https://github.com/Sylphie-Labs/memory-pkg) and [`@sylphie-labs/codebase-pkg`](https://github.com/Sylphie-Labs/codebase-pkg) — so any other project, sylphie included, can pull them in as a normal npm dependency.
