...
> `memory-pkg` addresses the episodic side of agent forgetting: the agent's lack of memory for the work itself. For the structural side (the agent's lack of a map of the codebase), see [sylphie-pkg](/cutting-edge-tech/sylphie-pkg), the complementary piece.

## What `memory-pkg` is

`memory-pkg` is a TimescaleDB-backed long-term session memory store, published as the standalone repo [`Sylphie-Labs/memory-pkg`](https://github.com/Sylphie-Labs/memory-pkg). Its own description: *"Long-term session memory for Claude Code work. JSONL buffer + TimescaleDB hypertable + fuzzy search + temporal windowing and unwind."* ([`package.json`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/package.json))

Six pieces hold it together:

1. **A capture hook** on `Stop` that reads the official transcript tail and appends new events to a local JSONL buffer. ([`template/.claude/hooks/memory-capture.cjs`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/template/.claude/hooks/memory-capture.cjs))
2. **An ingester** that rotates the buffer atomically and bulk-inserts into a Timescale hypertable. ([`src/ingest/ingester.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/ingest/ingester.ts))
3. **An injection hook** on `UserPromptSubmit`: a ~30-line wrapper that spawns the retrieval CLI, captures its stdout, and emits the result as `additionalContext` for the model's turn. **The hook never blocks the user's message.** If the CLI isn't built, if the DB is unreachable, if the pipeline times out, if anything errors at any layer, the hook silently exits and the user's prompt proceeds untouched. Fails open by design. ([`template/.claude/hooks/memory-inject.cjs`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/template/.claude/hooks/memory-inject.cjs))
4. **A multi-tier retrieval pipeline** behind that hook. It includes an **entity tier that reads the last 20 lines of the active transcript** so pronoun-shaped prompts like *"ok let's do that"* still find the right history. ([`src/inject/generate.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/generate.ts), [`src/inject/tiers/entity.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/entity.ts))
5. **An MCP server** with four tools (`searchMemory`, `getMemoryContext`, `unwindFromEvent`, `getSessionTimeline`) registered in the consumer's `.mcp.json` so the agent can deliberately drill in when auto-injection isn't enough. ([`src/mcp-server/index.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/mcp-server/index.ts))
6. **A rationale-synthesis job** that compresses each turn into a 2-3 sentence "why" event at ingest time, so future searches for "why did we…" match the reasoning instead of the actions. ([`src/rationale/synthesize.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/rationale/synthesize.ts))
...
Different consumers (display, injection, search, replay) each pick the cheapest representation sufficient for their job. The hot retrieval path never has to scan full event bodies; the fuzzy search only touches a 2 KB column; the injection block only pulls a 600 char excerpt. The same content stored four ways is what makes the cost story work. ([`memory-capture.cjs`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/template/.claude/hooks/memory-capture.cjs))

The buffer rotates atomically before ingest; failed batches move to `buffer.failed.jsonl` rather than being silently dropped. ([`src/ingest/ingester.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/ingest/ingester.ts)) Deduplication is enforced by a unique index on `(session_id, transcript_uuid, ts)`. ([`src/schema.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/schema.ts))
...
The schema script is idempotent: safe to re-run on every deploy. Full DDL: [`src/schema.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/schema.ts).

## The retrieval pipeline

Every new user prompt fires the injection hook ([`memory-inject.cjs`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/template/.claude/hooks/memory-inject.cjs)), which spawns the compiled CLI with a 30-second timeout (`DRIFT_MEMORY_HOOK_TIMEOUT_MS`-overridable) and emits the CLI's stdout as `additionalContext` on a `UserPromptSubmit` event. The hook fails open at every layer: if anything errors, the user's prompt proceeds untouched.

Inside the CLI, `generateInjection()` runs a tiered retrieval system. Five tiers are implemented; the live registry returns `[trigramTier, entityTier]` for the fast path and an empty rescue array. ([`src/inject/tiers/index.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/index.ts))

### The two live tiers

**Trigram lexical** ([`src/inject/tiers/trigram.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/trigram.ts)). Pure SQL. Postgres `word_similarity(query, search_text)` against the GIN trigram index. `word_similarity` finds the query's best-matching extent inside a longer document, the right metric for short prompts against longer event records. Pulls top 20 above 0.2 similarity, ordered by score then recency. Zero LLM cost.

**Entity-aware lexical** ([`src/inject/tiers/entity.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/entity.ts)). Same trigram engine, different inputs. Pure trigram fails when the prompt is a pronoun: *"ok let's do that"* contains no extractable concept. The entity tier extracts identifiers (backticked terms, double-quoted phrases, file paths, CamelCase, snake_case) from both the current prompt *and the last 20 lines of the active transcript*. Identifiers from the prompt are weighted double over transcript mentions; the top-ranked entities get one `word_similarity` query each, in parallel. Metadata is surfaced (queried entities, dropped ones, overflow counts) so the orchestrator can hint *"more matches exist for `<entity>`; call `searchMemory` to widen."* ([`src/inject/generate.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/generate.ts))

### The three dormant tiers

**Embedding** ([`src/inject/tiers/embedding.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/embedding.ts)). Would embed the query locally via `Xenova/bge-small-en-v1.5` (384-dim, ONNX, no API cost) and run an HNSW-accelerated cosine KNN against the `embedding` column. Off by default because the live data shows trigram + entity already meets the strong threshold for well-formed prompts, and the embedding tier adds 30–80 ms per call after a 1–2 s cold-load on first use. Schema and per-event embeddings are still maintained so this can be flipped on by editing the registry.

**Classifier-driven** ([`src/inject/tiers/classifier.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/classifier.ts), [`src/inject/classify.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/classify.ts)). Would shell out to a local `claude` CLI (Haiku via Max OAuth) for strict-JSON classification `{intent, subsystems, files, entities, confidence}`, validate the results against the filesystem and a subsystem whitelist, then query by subsystem tag and file-path suffix with classifications cached on disk for 24 hours. Off because it adds 6–10 s on a cold call and the trigram + entity path covers the same ground for most prompts. Available when you need entity-aware retrieval that goes beyond what regex-extraction can catch.

**Knowledge graph** ([`src/inject/tiers/kg.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/kg.ts)). Would read the classifier's cached file list, expand each file by one hop through `codebase-pkg`'s Neo4j `IMPORTS` edges, and query `memory_events` for events touching those files. Off because it depends on the classifier (also dormant) and the two-DB coordination is expensive for a tier whose contribution is by construction discounted (`× 0.7`). Useful for cross-module retrieval: "find memory events on files structurally related to the one we're discussing." Reactivating it is two registry edits.

### The merger

Each tier returns `{event_id, score, source_tier}` candidates. The merger ([`src/inject/merger.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/merger.ts)) combines them by weighted average (default strategy; alternatives: `union`, `intersection`). Default weights: trigram 0.2, entity 0.3, embedding 0.3, classifier 0.4, kg 0.1. Only the first two weights are active in the live config; the others describe what the merger would do if the rescue tiers were re-enabled. An event surfaced by multiple tiers wins on agreement, not just raw score.

**Fast-path short-circuit.** If trigram + entity produce a merged candidate at score ≥ 0.7, the rescue phase is skipped entirely: no Haiku call, no graph expansion. ([`src/inject/generate.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/generate.ts)) For well-formed prompts the answer comes back as pure SQL.
...
(Score ranges are illustrative; actual values depend on what's in your store. The pipeline shape is faithful to the code in [`src/inject/generate.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/generate.ts).)
...
That's all Claude Code needs to discover the server, spawn it as a child process, and call its tools. See the published server entrypoint: [`src/mcp-server/index.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/mcp-server/index.ts).
...
This is what rationale synthesis fixes. A separate post-ingest job walks through turns and synthesizes a 2-3 sentence "why" summary, inserting it as a `turn_rationale` event in the same searchable hypertable. ([`src/rationale/synthesize.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/rationale/synthesize.ts))
...
**What you pay.** Storage is cheap: Postgres on Docker, hypertable chunking keeps old chunks compressible, multi-resolution event storage keeps the indexed columns small. Embedding inference is zero in dollars. `bge-small-en-v1.5` is local CPU; the embed module documents ~30–80 ms per call after warmup with a ~1–2 s cold-load on the first call ([`src/embed.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/embed.ts); values stated in source, not independently measured). LLM retrieval cost in the live config is **zero**: trigram and entity tiers are pure SQL. If you enable the classifier or rerank tier, both use Haiku via the local `claude` CLI under Max OAuth, no API key consumed, classifications cache for 24 hours. Rationale synthesis is the same pattern: Haiku via Max, one call per turn at ingest, amortized forever after.

Latency budget (documented in [`memory-inject.cjs`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/template/.claude/hooks/memory-inject.cjs) as a design target, not measured here):
...
- **Continuous aggregates and compression.** Timescale supports both; the [README](https://github.com/Sylphie-Labs/memory-pkg/blob/main/README.md) roadmap notes them as future work. Today the table grows linearly.
- **Vector embedding backfill at scale.** The `backfillEmbeddings()` helper exists ([`src/embed.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/embed.ts)) but is meant to be run deliberately; nothing automatically enforces that every event gets embedded.
- **A hands-on install tutorial.** The package is published as [`@sylphie-labs/memory-pkg`](https://github.com/Sylphie-Labs/memory-pkg) with a CLI lifecycle (`init`, `upgrade`, `status`, `doctor`, `uninstall`), but a step-by-step walkthrough for adding it to a new repo is still owed.
...
Repo: [`Sylphie-Labs/memory-pkg`](https://github.com/Sylphie-Labs/memory-pkg). Consumer-side hooks ship under `template/.claude/hooks/` and are copied into a project by `memory-pkg init`.

| Component | File |
|---|---|
| Schema | [`src/schema.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/schema.ts) |
| Capture hook | [`template/.claude/hooks/memory-capture.cjs`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/template/.claude/hooks/memory-capture.cjs) |
| Injection hook | [`template/.claude/hooks/memory-inject.cjs`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/template/.claude/hooks/memory-inject.cjs) |
| Ingester | [`src/ingest/ingester.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/ingest/ingester.ts) |
| Injection orchestrator | [`src/inject/generate.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/generate.ts) |
| Tier registry | [`src/inject/tiers/index.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/index.ts) |
| Trigram tier | [`src/inject/tiers/trigram.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/trigram.ts) |
| Entity tier | [`src/inject/tiers/entity.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/entity.ts) |
| Embedding tier | [`src/inject/tiers/embedding.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/embedding.ts) |
| Classifier tier | [`src/inject/tiers/classifier.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/classifier.ts) |
| KG tier | [`src/inject/tiers/kg.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/tiers/kg.ts) |
| Merger | [`src/inject/merger.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/inject/merger.ts) |
| Embedder | [`src/embed.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/embed.ts) |
| Rationale synthesis | [`src/rationale/synthesize.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/rationale/synthesize.ts) |
| MCP server | [`src/mcp-server/index.ts`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/src/mcp-server/index.ts) |
| README | [`README.md`](https://github.com/Sylphie-Labs/memory-pkg/blob/main/README.md) |

The complementary `codebase-pkg` package, which the (currently dormant) KG retrieval tier consults at `bolt://localhost:7688`, is also published standalone at [`Sylphie-Labs/codebase-pkg`](https://github.com/Sylphie-Labs/codebase-pkg). Both can be pulled into any project as normal npm dependencies; see the [`sylphie-pkg`](/cutting-edge-tech/sylphie-pkg) write-up for what the codebase graph does.
