...
> `sylphie-pkg` addresses the structural side of agent forgetting: the agent's lack of a map of the codebase. The episodic side is the agent's lack of memory for the work itself, the decisions and false starts and constraints that shape the next session. For that, see [memory-pkg](/cutting-edge-tech/memory-pkg), the complementary piece.
...
**`/sync-pkg`** is the user-facing entry point. It runs `yarn sync-pkg`, then `/classify-pkg-domains`, then `/infer-pkg-connections`. Each step gates on the previous one's success. ([`template/.claude/skills/sync-pkg/SKILL.md`](https://github.com/Sylphie-Labs/codebase-pkg/blob/main/template/.claude/skills/sync-pkg/SKILL.md))
