...
> This article is about the per-write and per-turn review hooks in this portfolio repo. For the related pattern of using hooks to inject long-term memory into prompts, see [memory-pkg](/work/memory-pkg). For the graph-backed map of a codebase that the agent queries instead of re-reading, see [sylphie-pkg](/work/sylphie-pkg).
...
Compare these review hooks with the injection hook described in the [memory-pkg](/work/memory-pkg) write-up. That hook *fails open*. If the retrieval CLI errors, if the DB is down, if anything goes wrong, the hook silently exits and the user's prompt proceeds untouched. Memory enrichment is a nice-to-have; not blocking the user is the priority.
