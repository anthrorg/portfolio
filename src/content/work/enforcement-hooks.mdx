...
> This article is about the per-write and per-turn review hooks in this portfolio repo. For the related pattern of using hooks to inject long-term memory into prompts, see [memory-pkg](/cutting-edge-tech/memory-pkg). For the graph-backed map of a codebase that the agent queries instead of re-reading, see [sylphie-pkg](/cutting-edge-tech/sylphie-pkg).
...
Compare these review hooks with the injection hook described in the [memory-pkg](/cutting-edge-tech/memory-pkg) write-up. That hook *fails open*. If the retrieval CLI errors, if the DB is down, if anything goes wrong, the hook silently exits and the user's prompt proceeds untouched. Memory enrichment is a nice-to-have; not blocking the user is the priority.
...
**Different jobs, different defaults. Inject-style hooks should fail open because their value is additive. Review-style hooks should fail safe because their value is gating.** Every choice in the rest of this article falls out of that axis: what happens when the reviewer times out, when the regex misses a file, when the transcript can't be found.
...
>
> Do this in order:
> 1. Run `git diff HEAD` and `git ls-files --others --exclude-standard` to see exactly what changed.
