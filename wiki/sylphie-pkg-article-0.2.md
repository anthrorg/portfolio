...
*`sylphie-pkg` addresses the structural side of agent forgetting — the agent's lack of a map of the codebase. For the episodic side — the agent's lack of memory for the work itself, the decisions and false starts and constraints that shape the next session — see [memory-pkg](./long-term-memory-article.md), the complementary piece.*
...
**Effect 2 — questions you can't ask with grep.** Hub detection, pipeline discovery, and cross-package bridge analysis are not searches over text. They're graph queries. The pipeline-detection query in `/infer-pkg-connections` is something like `MATCH (a:Function)-[:CALLS]->(b:Function)-[:CALLS]->(c:Function)-[:CALLS]->(d:Function)` — one Cypher line. ([`infer-pkg-connections/SKILL.md:74-83`](https://github.com/)) The equivalent through file-reading would be many turns, with the agent re-discovering the same call graph each time.
