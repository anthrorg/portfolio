# A Knowledge Graph Of Your Own Codebase

**How `sylphie-pkg` turned a monorepo into a queryable graph that Claude Code can navigate without ever reading a file.**

*Design and implementation walkthrough, not an install tutorial. Install steps will land when the NPM package ships; for now the code lives in two repos and is referenced here by file path.*
...
`sylphie-pkg` was built to solve that specific problem in that specific repo. The abstraction turned out to be more general than the use case — once the code was working, porting it to a second project (drift-detector) took about thirty minutes. Same pattern, different config. That kind of accidental portability is the signal that says the abstraction is right.

## What `sylphie-pkg` is

`sylphie-pkg` is a small monorepo package that maintains a Neo4j knowledge graph of the codebase. ([`sylphie/packages/sylphie-pkg/package.json`](https://github.com/) — `@sylphie/sylphie-pkg`, version `0.1.0`) The package's own `package.json` describes it succinctly: *"Codebase Personal Knowledge Graph — queryable map of the entire codebase for Claude Code agents."* ([`packages/sylphie-pkg/package.json:6`](https://github.com/))

Three jobs:

1. **Parse the source tree** with ts-morph and extract structured metadata about every function, type, import, class hierarchy, and constructor injection. ([`packages/sylphie-pkg/src/sync/ast-parser.ts`](https://github.com/))
2. **Maintain the graph incrementally** by diffing against the last synced git commit, hashing function/type bodies for change detection, and applying only the deltas under one atomic transaction. ([`packages/sylphie-pkg/src/sync/sync-pipeline.ts`](https://github.com/))
3. **Expose the graph to Claude Code as an MCP server** — seven query tools over stdio, registered in [`sylphie/.mcp.json`](https://github.com/) so Claude Code spawns it as a child process at session start. ([`packages/sylphie-pkg/src/mcp-server/index.ts:43-198`](https://github.com/))

A drift-detector port (`@drift/codebase-pkg`, [`drift-detector/packages/codebase-pkg/`](https://github.com/)) carries the same pattern into a different product. Same parser, same Cypher, same MCP shape, different config and different repo.
...
The initial seed is the same pipeline run with an empty graph and a watch-all file scan, with explicit schema setup creating 4 uniqueness constraints and 11 indexes. ([`initial-seed.ts:146-164`](https://github.com/))

## Three skills that drive the loop

A Neo4j graph alone is not useful. What makes `sylphie-pkg` work day-to-day is three Claude Code slash commands that operate on it.

**`/sync-pkg`** — user-facing entry point. Runs `yarn sync-pkg`, then `/classify-pkg-domains`, then `/infer-pkg-connections`. Each step gates on the previous one's success. ([`.claude/skills/sync-pkg/SKILL.md`](https://github.com/))

**`/classify-pkg-domains`** — assigns each `Function` node a domain label from the 14-value `DOMAIN_LABELS` enum: `decision-making`, `communication`, `learning`, `drive-engine`, `planning`, `knowledge-graph`, `event-backbone`, `database`, `web-api`, `metrics`, `orchestration`, `shared-utilities`, `testing`, plus an `unclassified` default state set on every new `Function` node via `coalesce(f.domain, 'unclassified')` ([`mutation-builder.ts:44`](https://github.com/)). The skill assigns one of the other thirteen, using directory location, JSDoc, and signature as inputs. ([`.claude/skills/classify-pkg-domains/SKILL.md`](https://github.com/), [`domain-classifier.ts:17-32`](https://github.com/))

**`/infer-pkg-connections`** — runs six analytical Cypher queries: hub detection (highly connected functions/types), pipeline discovery (3+ hop CALLS chains across files), cross-package bridge detection, circular-dependency detection at module and function level, dead-code detection (with explicit caveats for decorator-driven entry points), architectural-layer inference. ([`.claude/skills/infer-pkg-connections/SKILL.md`](https://github.com/)) Results write back as new graph properties and edges (`DATA_FLOWS_TO`, `BRIDGES`, `hubScore`, `possiblyDead`, `architecturalLayer`).
...
There's an architectural choice in both skills that's quietly load-bearing for the entire cost story. Both `SKILL.md` files specify *"No LLM API calls — YOU are the classifier"* / *"YOU are the analyzer"*. ([`classify-pkg-domains/SKILL.md:101`](https://github.com/), [`infer-pkg-connections/SKILL.md:255`](https://github.com/)) What that means concretely: the skills never spawn a Haiku or Sonnet call, never consume an `ANTHROPIC_API_KEY`, never run a separate inference job. Classification, hub detection, pipeline discovery, dead-code analysis — all of it gets done by the Claude Code session the developer is *already running*.
...
The MCP server exposes seven tools over stdio. Wiring is one stanza of [`sylphie/.mcp.json`](https://github.com/):
...
**Effect 2 — questions you can't ask with grep.** Hub detection, pipeline discovery, and cross-package bridge analysis are not searches over text. They're graph queries. `MATCH (a:Function)-[:CALLS]->(b:Function)-[:CALLS]->(c:Function)-[:CALLS]->(d:Function)` is one Cypher line. The equivalent through file-reading would be many turns, with the agent re-discovering the same call graph each time. ([`.claude/skills/infer-pkg-connections/SKILL.md`](https://github.com/))

## What this costs

The honest answer is: less than the alternative, but not zero.

**Compute.** The sync pipeline runs `git diff`, parses changed files with ts-morph, writes Cypher. The initial seed reports its parse-time and relationship-pass time explicitly. ([`initial-seed.ts:521-525, 630-634`](https://github.com/)) Files process in batches of 50 with the ts-morph project cache cleared every 5 batches to keep memory bounded. ([`initial-seed.ts:44, 515`](https://github.com/)) Incremental syncs are dominated by Neo4j round-trips, not parsing.
...
- **Multi-language.** Today it's TypeScript and TSX only. The `ast-parser.ts` is ts-morph-bound. Drift-detector's roadmap explicitly mentions tree-sitter integration for Python, Go, Rust as the next step. ([`drift-detector/packages/codebase-pkg/README.md:82`](https://github.com/))
- **Test coverage.** Integrity checks substitute for unit tests on the sync pipeline. The drift-detector port carries that forward as known tech debt. ([`drift-detector/packages/codebase-pkg/README.md:81`](https://github.com/))
...
`sylphie-pkg` doesn't make the agent smarter. It hands the agent a map instead of a flashlight. The developer stops being the agent's index; the session stops being a tourist's recon. That's the whole pitch.
